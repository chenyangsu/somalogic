...
########## Description ##########
# Runs one Logistic regression model per protein with the formula = comorbiditiy ~ protein to get results for each protein (sorted by decreasing p value, lowest p to highest p): 
...
if (data == "infe") {  # set dat
  dat <- read.csv(file = "./results/datasets/comorbidities_infe_417-soma_data=normalized-nat_log_tranf=TRUE-standardize=TRUE-remove_outliers=FALSE.csv")
...
for (prot in prot_list) {
  ...
  if (outcome == "com_diabetes") {
    formula <- com_diabetes ~ protein 
    
  } else if (outcome == "com_chronic_pulm") {
    formula <- com_chronic_pulm ~ protein 
    
  } else if (outcome == "com_chronic_kidney") {
    formula <- com_chronic_kidney ~ protein 
    
  } else if (outcome == "com_heart_failure") {
    formula <- com_heart_failure ~ protein 
    
  } else if (outcome == "com_hypertension") {
    formula <- com_hypertension ~ protein 
    
  } else if (outcome == "com_liver") {
    formula <- com_liver ~ protein 
  ...
  p <-sum_stat$coefficients[2, 4]  # get p value of protein
  or <- a[2, 1]  # get odds ratio of protein
  ci_low <- a[2, 2]  # get lower confidence interval of protein
  ci_high <- a[2, 3]  # get higher confidence interval of protein
...
file_name <- paste("./results/all_proteins/comorbidity~protlevel/", data, outcome, "LR",            
                   "protlevel", "Analysis=all_proteins.xlsx", sep = "_")  # concatenates arguments by separator
